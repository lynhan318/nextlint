...
{#if visible}
  <div
    class="absolute inset-x-0 flex justify-center top-[-24px]"
  ...
  >
    ...
    >
      <button
        ...
        on:mousedown|stopPropagation={() => updateAttributes({fit: 'contain'})}
        class={cn(
      ...
      <button
        aria-label="Fit View"
        on:mousedown|stopPropagation={() => updateAttributes({fit: 'cover'})}
        class={cn(
...
<img
  ...
  on:mousedown|preventDefault|stopPropagation={selectNode}
  class={cn('border-none rounded-md')}
  contenteditable="false"
/>

<figcaption use:contentDOM />
